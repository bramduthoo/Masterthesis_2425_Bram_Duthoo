...
Gevoeligheidsanalyse van 1e-3 tot 1e-7 geeft weinig verandering in PCA plot en verandert structuur van data dus niet veel. Gebruik van 1e-4 is dus verantwoord.
...
pca_result <- prcomp(clr_data, center = TRUE, scale. = FALSE)
summary(pca_result)

# Voeg smaaklabels toe
clr_df <- as.data.frame(clr_data)
clr_df$flavor_label <- data_grouped$flavor_label  # Voeg labels toe

# PCA uitvoeren
pca_result <- prcomp(clr_df %>% dplyr::select(-flavor_label), center = TRUE, scale. = FALSE)

# PCA-plot maken met kleur per smaak
library(ggplot2)
library(ggfortify)

autoplot(pca_result, data = clr_df, colour = "flavor_label", loadings.label.size = 3) +
  ggtitle("PCA (CLR-transformed data with multiplicative zero replacement)") +
  theme_minimal()
```
